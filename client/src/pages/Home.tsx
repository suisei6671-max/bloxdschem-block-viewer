...
function buildResult(fileName: string, schematic: ReturnType<typeof parseSchematic>, blockDatas: BlockData[], ja: Record<string, string>, en: Record<string, string>): Result {
  ...
  counts.forEach((count, id) => { const data = blockDatas[id - 1]; const rootId = data?.rootId; if (typeof rootId === "number" && rootId !== id) { counts.set(id, 0); counts.set(rootId, (counts.get(rootId) || 0) + count); } });
  const rows = Array.from(counts.entries()).filter(([id, count]) => id > 0 && count > 0 && blockDatas[id - 1]?.name !== "Unloaded").sort((a, b) => a[0] - b[0]).map(([id, count]) => { const data = blockDatas[id - 1]; const resolved = resolveDisplay(data, ja, en); if (!data) warnings.push(`ID ${id} のblockDataがありません。`); return { id, count, data, ...resolved }; });
